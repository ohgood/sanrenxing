...
import com.sanrenxing.shop.service.AlipayTradeService;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    private Logger logger = LoggerFactory.getLogger(AbsAlipayTradeService.class);
    ...
    protected AlipayTradeQueryResponse tradeQuery(AlipayTradeQueryRequestBuilder builder) {
        ...
        logger.info("runner.query bizContent:" + request.getBizContent());
    ...
    public AlipayF2FRefundResult tradeRefund(AlipayTradeRefundRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        logger.info("runner.refund bizContent:" + request.getBizContent());
    ...
    public AlipayF2FPrecreateResult tradePrecreate(AlipayTradePrecreateRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        logger.info("runner.precreate bizContent:" + request.getBizContent());
    ...
    protected AlipayTradeCancelResponse tradeCancel(AlipayTradeCancelRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        logger.info("runner.cancel bizContent:" + request.getBizContent());
    ...
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        ...
        for (int i = 0; i < alipay.getMaxQueryRetry(); i++) {
            Utils.sleep(alipay.getQueryDuration());
    ...
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < alipay.getMaxCancelRetry(); i++) {
                    Utils.sleep(alipay.getCancelDuration());
